...
class _FludoGameState extends State<FludoGame> with TickerProviderStateMixin {
                          ...
                          SizedBox.expand(
                            ...
                            builder: (_, __) => StreamBuilder(
                              stream: Firestore.instance
                                  .collection('_currentTurn')
                                  .snapshots(),
                              builder: (ctx, snapshot) => CustomPaint(
                                painter: OverlaySurface(
                                    highlightColor: _playerHighlightAnim.value,
                                    selectedHomeIndex: snapshot
                                        .data.documents.first['current_Turn'],
                            ...
                            return SizedBox.expand(
                              child: StreamBuilder(
                                ...
                                builder: (ctx, snapshot) {
                                  int output = snapshot
                                      .data.documents.first['dice_output'];
        ...
        playerPawns.add(SizedBox.expand(
          child: AnimatedBuilder(
            builder: (_, child) => StreamBuilder(
              stream:
                  Firestore.instance.collection('PlayersPosition').snapshots(),
              builder: (ctx, snapshot) {
                Offset currentspot = Offset(
                    double.parse(snapshot.data.documents
                        .first['0'][playerIndex][pawnIndex.toString()][0]
                        .toString()),
                    double.parse(snapshot.data.documents
                        .first['0'][playerIndex][pawnIndex.toString()][1]
                        .toString()));
                // print(currentspot);
                //
                ...
                //   currentStepIndex =
                //       max(_pawnCurrentStepInfo[playerIndex][pawnIndex].key, 0);
                // } else {
                //   currentStepIndex = min(
                //       _pawnCurrentStepInfo[playerIndex][pawnIndex]
                //           .key, //condition to avoid incrementing key for initial step
                //       _maxTrackIndex);
                // }

                // print(_playerAnimList[playerIndex][pawnIndex].value.toString() +
                //     '  before update');
                // var animCont = _playerAnimContList[playerIndex][pawnIndex];
                // _playerAnimList[playerIndex][pawnIndex] = Tween(
                //         begin: _playerTracks[playerIndex][pawnIndex]
                //                 [currentStepIndex]
                //             .center,
                //         end: currentspot)
                //     .animate(animCont);
  ...
  _movePawn({bool considerCurrentStep = false}) {
    ...
    if (_collisionDetails.isReverse) {
      ...
      //     data[player]['${pawn.toString()}'] = [
      //       _playerAnimList[player][pawn].value.dx,
      //       _playerAnimList[player][pawn].value.dy
      //     ];
      //     print(player);
      //     print(pawn);
      //     print(_playerAnimList[player][pawn]);
      //   }
      // }
      // Firestore.instance
      //     .collection('PlayersPosition')
      //     .document('oyD87SESFbLlht5POF3w')
      //     .updateData({"0": data});
    } else {
    ...
    for (int player = 0; player < 4; player++) {
      for (int pawn = 0; pawn < 4; pawn++) {
        ...
        print(player);
        print(pawn);
        print(_playerAnimList[player][pawn].value);
      }
    }
    Firestore.instance
        .collection('PlayersPosition')
...

    {
      ...
      if (!_provideFreeTurn) {
        do {
          ...
          Firestore.instance
              .collection('_currentTurn')
              .document('56Ywe7B0VCaMNPxuYqiY')
              .updateData({"current_Turn": _currentTurn});
          if (_winnerPawnList[_currentTurn].length != 4)
